...
library(ggplot2)
library(scales)
source("/home/zhepan/Project/scRNA_BC_metastases/Analysis/scRNA/func/custom_magma.R")
source("/home/zhepan/Project/scRNA_BC_metastases/Analysis/scRNA/func/robust_nmf_programs.R")

Genes_nmf_w_basis = readRDS("~/Project/scRNA_BC_metastases/Result/nmf/tumor/nmf_gene_score_0_02.rds")
...
# NMF selection
n_genes = 50
intra_min_parameter <- 35 # minimum overlap with a program from the same cell line (for selecting robust programs)
intra_max_parameter <- 15 # maximum overlap with a program from the same cell line (for removing redundant programs)
inter_min_parameter <- 15 # minimum overlap with a program from another cell line 
...
# 2) Selected NMFs are then clustered iteratively, as described in Figure S1. At the end of the process, each cluster generates a list of the n_genes genes (i.e. the MP) that represent the NMF programs that contributed to the cluster. Notably, not all initially selected NMFs end up participating in a cluster  
...
# get top n_genes genes for each NMF program 
nmf_programs          <- lapply(Genes_nmf_w_basis, function(x) apply(x, 2, function(y) names(sort(y, decreasing = T))[1:n_genes]))
...
nmf_intersect_hc     <- hclust(as.dist(n_genes-nmf_intersect), method="average") 
...
while (Sorted_intersection[1]>Min_group_size) {  
  ...
  Genes_MP                    <- nmf_programs[,names(Sorted_intersection[1])] # Genes in the forming MP are first chosen to be those in the first NMF. Genes_MP always has only n_genes genes and evolves during the formation of the cluster
  ...
  ### Create gene list is composed of intersecting genes (in descending order by frequency). When the number of genes with a given frequency span bewond the n_genesth genes, they are sorted according to their NMF score.    
  while ( Intersection_with_Genes_MP[1] >= Min_intersect_cluster) {  
    ...
    Genes_at_border <- Genes_MP_temp[which(Genes_MP_temp == Genes_MP_temp[n_genes])]   ### genes with overlap equal to the n_genesth gene
    ...
    if (length(Genes_at_border)>1){
      ...
      for (i in Curr_cluster) {
        curr_study           <- stringr::str_sub(i,1,-29)
...
ggplot(data = nmf_intersect_meltI_NEW, aes(x=Var1, y=Var2, fill=100*value/(100-value), color=100*value/(100-value))) + 
  geom_tile() + 
  #geom_rect(aes(xmin = 0, xmax = length(Cluster_list$Cluster_1), fill = NA),ymin = 0,ymax = length(Cluster_list$Cluster_1))+
  #geom_hline(aes(yintercept = length(Cluster_list$Cluster_1)))+
  #geom_vline(aes(xintercept = length(Cluster_list$Cluster_1)))+
  geom_hline(aes(yintercept = length(inds_new)-length(inds_sorted)),size = 0.3)+
  geom_vline(aes(xintercept = length(inds_sorted)),size = 0.3)+
